...
export interface Stats { total_questions: number; unique_seen: number; unique_remaining: number; coverage_percent: number; total_attempts: number; correct: number; incorrect: number; overall_accuracy: number; first_attempt_accuracy: number; recent_accuracy: number; passing_grade_percent: number; passing_difference: number; mistakes: Record<MistakeType, number>; categories: CategoryStat[]; weakest_categories: CategoryStat[]; strongest_categories: CategoryStat[]; missed_twice: MissedQuestion[] }
