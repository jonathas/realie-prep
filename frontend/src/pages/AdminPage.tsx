...
export function AdminPage() {
  ...
  return <section className="mx-auto max-w-5xl px-5 py-10 lg:py-14">
    <p className="eyebrow">Administration</p><h1 className="mt-2 font-serif text-4xl font-bold sm:text-5xl">Question bank</h1><p className="mt-3 max-w-2xl text-black/50">{status.filename ? 'Check the official NPI website for updates at any time. Changes are validated and previewed before replacing your local bank.' : 'RealiePrep does not distribute the official questions. Download them directly from NPI into this installation’s private local data volume.'}</p>
    ...
    <div className="mt-8 grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
      ...
      <div className="card p-6 sm:p-8"><RefreshCw className={`text-sage-700 ${busy ? 'animate-spin' : ''}`} size={30}/><h2 className="mt-4 font-serif text-2xl">{status.filename ? 'Check for updates' : 'Download from NPI'}</h2><p className="mt-2 text-sm leading-relaxed text-black/50">RealiePrep fetches the bank directly from the official website only when you request it, caches it locally, and validates all 300 questions and 30 topics. NPI ČR retains copyright in the downloaded material.</p>{!status.filename && <label className="mt-5 flex items-start gap-3 rounded-xl bg-sage-50 p-4 text-sm text-black/60"><input className="mt-1 h-4 w-4 accent-sage-700" type="checkbox" checked={sourceAccepted} onChange={(event) => setSourceAccepted(event.target.checked)}/><span>I want to download the official question bank for local self-study. <a className="font-semibold text-sage-700 underline" href="https://cestina-pro-cizince.cz/obcanstvi/databanka-uloh/" target="_blank" rel="noreferrer">View the official source</a>.</span></label>}<button className="btn-secondary mt-6 w-full" disabled={busy || (!status.filename && !sourceAccepted)} onClick={() => void checkForUpdates()}><RefreshCw size={17}/>{busy ? 'Contacting official source…' : status.filename ? 'Check official source' : 'Download official bank'}</button></div>
    </div>
    {preview && <div className="card mt-6 p-6 sm:p-8"><div className="flex items-center gap-3"><CheckCircle2 className={preview.valid ? 'text-sage-600' : 'text-red-600'}/><div><p className="eyebrow">Download preview</p><h2 className="font-serif text-2xl">{preview.filename}</h2></div></div><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-xl bg-sage-50 p-4"><strong>{preview.question_count} / 300</strong><p className="text-xs text-black/45">Questions</p></div><div className="rounded-xl bg-sage-50 p-4"><strong>{preview.category_count} / 30</strong><p className="text-xs text-black/45">Topics</p></div><div className="rounded-xl bg-sage-50 p-4"><strong>{preview.option_count}</strong><p className="text-xs text-black/45">Options</p></div></div>{preview.errors.length > 0 && <ul className="mt-4 list-disc pl-5 text-sm text-red-700">{preview.errors.map((item) => <li key={item}>{item}</li>)}</ul>}<p className="mt-5 text-sm text-amber-700">{status.filename ? 'Confirming an update replaces the local bank and resets study progress so statistics remain consistent.' : 'Confirmation stores this validated copy only in your local RealiePrep data volume.'}</p><button className="btn-primary mt-4" disabled={!preview.valid || busy} onClick={() => void confirm()}>{status.filename ? 'Confirm update' : 'Install local question bank'}</button></div>}
  </section>
}
